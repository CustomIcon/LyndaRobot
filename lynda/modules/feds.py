...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Can't federate! Please contact @LyndaEagleSupport if the problem persist.")
			return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			message.reply_text("Failed to join federation! Please contact @LyndaEagleSupport should this problem persist!")
			return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
	...
	if fban:
		...
		if not x:
			message.reply_text("Failed to ban from the federation! If this problem continues, contact @LyndaEagleSupport.")
			return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
	...
	if len(args) >= 1:
		...
		if not x:
			update.effective_message.reply_text("Whoa! There was an error while setting federation rules! If you wondered why please ask it in @LyndaEagleSupport !")
			return
	...
	if args:
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="kigyo_fbanned_users.json",
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="kigyo_fbanned_users.csv",
