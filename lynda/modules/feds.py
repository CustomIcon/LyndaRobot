...
def new_fed(bot: Bot, update: Update):
    ...
    if chat.type != "private":
        update.effective_message.reply_text(
            "Make your federation in my PM, not in a group.")
        return
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @Aman_Ahmed if the problem persists.")
            return
...
def del_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if chat.type != "private":
        update.effective_message.reply_text(
            "Delete your federation in my PM, not in the group.")
        return
    if args:
        is_fed_id = args[0]
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @Aman_Ahmed should this problem persists!")
            return
        ...
        message.reply_text(
            "This chat has joined the federation: {}!".format(
                getfed['fname']))
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @Aman_Ahmed.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Big F! There is an error while setting federation rules! If you wondered why please ask it to @Aman_Ahmed !")
            return
