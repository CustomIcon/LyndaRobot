...
@user_admin
def add_warn_filter(update: Update, _):
    chat: Optional[Chat] = update.effective_chat
...
@user_admin
def remove_warn_filter(update: Update, _):
    chat: Optional[Chat] = update.effective_chat
    msg: Optional[Message] = update.effective_message
...
@run_async
def list_warn_filters(update: Update, _):
    chat: Optional[Chat] = update.effective_chat
...
@run_async
@loggable
def reply_filter(update: Update, _) -> str:
    chat: Optional[Chat] = update.effective_chat
    message: Optional[Message] = update.effective_message
...
def __stats__():
    return (
        f"-> <code>{sql.num_warns()}</code> overall warns, across <code>{sql.num_warn_chats()}</code> chats.\n"
        f"-> <code>{sql.num_warn_filters()}</code> warn filters, across <code>{sql.num_warn_filter_chats()}</code> chats.")
