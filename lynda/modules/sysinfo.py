...
def seconds_to_str(seconds):
    ...
    return "{} day(s), {:02} hours, {:02} minutes, {:02} seconds".format(
...
def sysinfo(bot: Bot, update: Update):
    ...
    cpu_info = "{}\n\n{}\nPhysical cores: {}\nTotal cores: {}\nMax Frequency: {:.2f} Mhz\nCurrent Frequency: {:.2f} Mhz\nCPU Usage: {}%\n".format(
        cpu_header, cpumodel, psutil.cpu_count(
            logical=False), psutil.cpu_count(
            logical=True), cpufreq.max, cpufreq.current, psutil.cpu_percent(
                percpu=False, interval=1))
    d = "=" * 15, " RAM ", "=" * 15
    ram_header = "".join(d)
