    ...
    for chat in chats:
        ...
        if ((100 * chats.index(chat)) / len(chats)) > progress:
            ...
            if progress_message:
                ...
                progress_message = context.bot.sendMessage(chat_id, progress_bar)
            progress += 5

        cid = chat.chat_id
        sleep(0.1)
        try:
            context.bot.get_chat(cid, timeout=60)
...
def get_invalid_gban(context: CallbackContext, _, remove: bool = False):
...
def get_muted_chats(context: CallbackContext, update: Update, leave: bool = False):
    ...
    for chat in chats:
        ...
        try:
            context.bot.send_chat_action(cid, "TYPING", timeout=60)
        except (BadRequest, Unauthorized):
            muted_chats += +1
            chat_list.append(cid)
        except Exception as e:
    ...
    if leave:
        for muted_chat in chat_list:
            sleep(0.1)
            try:
                context.bot.leaveChat(muted_chat, timeout=60)
            except Exception as e:
                print(e)
            user_sql.rem_chat(muted_chat)
    return muted_chats
...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat" and query.from_user.id in admin_list:
        context.bot.editMessageText(
            ...
            chat_id,
            message.message_id)
        chat_count = get_muted_chats(context.bot, update, True)
