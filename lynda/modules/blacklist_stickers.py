	...
	if args:
		if args[0].lower() == 'off' or args[0].lower() == 'nothing' or args[0].lower() == 'no':
			...
			if len(args) == 1:
				...
				send_message(update.effective_message, teks, parse_mode="markdown")
				return
			settypeblacklist = send_message(update.effective_message, 'temporary muted for {}').format(args[1])
		...
		if to_match.set_name.lower() == trigger.lower():
			try:
				if getmode == 0:
					...
					message.delete()
					warn(update.effective_user, chat, "Using sticker '{}' which in blacklist stickers".format(trigger), message, update.effective_user)
					return
				elif getmode == 3:
					message.delete()
